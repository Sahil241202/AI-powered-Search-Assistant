...
main_agent_instructions = [
    ...
    # "IMPORTANT: Preserve the exact query as provided by the user without modification. Do not append years"
...
4. CRITICAL: For school or college queries, you MUST create a table with ALL these columns:
   - Institution Name (Latest Official Name)
   - Established Year (Verified from Multiple Sources)
   - Location (Current, Precise Location)
   - Type (Public/Private - Most Recent Status)
...
5. After searching, synthesize a complete response that integrates all information sources with the COMPLETE table structure
"""
...
class AlwaysSearchAgent(Agent):
    def run(self, message, **kwargs):
        ...
        enriched_message = f"IMPORTANT: YOU MUST USE SEARCH TOOLS and WebCrawler for this query but DO NOT add year on your own,but you are allowed to do other changes for functionality. Query: {message}"
